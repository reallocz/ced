...
struct buffer {
	unsigned int id; // 0 for unused buffer
	int flags;
...
struct buffer* _get_buffer(hBuffer buf)
{
	...
	for(unsigned int i = 0; i < G.bufcap; ++i) {
		if(G.buffers[i].id != BUFFER_DEFID) {
			log_ec("%d, ", G.buffers[i].id);
		}
	}
	log_ec("\n");
	// Buffer not found
	log_fatal(G.tag, "%s :buffer not found:%d. See above"
			, __func__, buf);
}


/** Generate a new id for buffer starting from 1 */
static
